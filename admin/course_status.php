...
?>
 ...
 $date=date("Y-m-d");
						  $x=1;
						$query ="SELECT a.*,b.*,c.* FROM course AS a, students AS b, course_subscription AS c WHERE a.id=c.cid AND b.sid=c.uid AND c.cid=a.id AND c.status=0";   
						  ...
						  ?>							   
                               <tr>  
									...
									<td><a href="course_substatus.php?id=<?php echo $row['id']; ?>" class=""><img src='images/edit.png' class='edit'></a> </td>
 ...
 $date=date("Y-m-d");
						  $x=1;
						 $query ="SELECT a.*,b.*,c.* FROM course AS a, students AS b, course_subscription AS c WHERE a.id=c.cid AND b.sid=c.uid AND c.cid=a.id AND c.status=1"; 
						  $result = mysqli_query($con, $query); 						  
                          while($row = mysqli_fetch_array($result))  
                          {  
						  ?>							   
...
<div class="tab-pane fade" id="profile-2" role="tabpanel" aria-labelledby="profile-tab">
 ...
 $date=date("Y-m-d");
						  $x=1;
				$query ="SELECT a.*,b.*,c.* FROM course AS a, students AS b, course_subscription AS c WHERE a.id=c.cid AND b.sid=c.uid AND c.cid=a.id";
						  $result = mysqli_query($con, $query); 						  
                          while($row = mysqli_fetch_array($result))  
                          {  
						  ?>							   
