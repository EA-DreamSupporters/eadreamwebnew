...
$query ="SELECT a.*,b.* FROM orders AS a, students AS b WHERE a.userId=b.sid AND a.orderNumber='$orderno'";  
...
<tr>
    ...
    <td><?php $ostatus=$row['orderStatus'];
                    if( $ostatus==''): echo "Not Processed Yet";
                            ...
                            <br />
<a href="javascript:void(0);" onClick="popUpWindow('track_order.php?oid=<?php echo $row['id'];?>');" title="Track order"> Track order
</a>
...
<tr>
    ...
    <td><address><?php echo htmlentities($row['address']);?>.
</address>
    </td>
</tr>
<tr>
...
<?php
$ret=mysqli_query($con,"select*from orders where orderStatus!='Delivered'&& orderStatus='' && orderNumber='$orderno'");
$nume=mysqli_num_rows($ret);
if($nume>0)
{
?>
<tr><td colspan="2"><a href="javascript:void(0);" onClick="popUpWindow('cancelorder.php?oid=<?php echo $row['id'];?>');" title="Cancel Order" class="btn-upper btn btn-danger">Cancel this Order
</a></td></tr>
<?php 
}
?>
...
<?php  ?>
    <div class="table-responsive">
        <table class="table">
            ...
            <tbody>
<?php
$query1 ="SELECT a.*,b.* FROM  ordersdetails AS a, books AS b WHERE a.orderNumber='$orderno' AND a.userId='$uid' AND a.productId=b.book_id";  
