...
$bok=mysqli_query($con,"SELECT * FROM books where cat_id='$categoryid'");
		 $tot= mysqli_num_rows($bok);			
    ?>
						<?php
$categoryid=$_GET['catid'];
$query=mysqli_query($con,"select * from books where cat_id='$categoryid'");
...
<a href="book_details.php?id=<?php echo $row['book_id']; ?>" class="btn btn-secondary box-btn btnhover2"> View Details</a>										</div>
									</div>
								</div>
							</div>
<?php 
